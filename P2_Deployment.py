...
if df is not None:
    ...
    with tab4:
        ...
        with col_right:
            ...
            # Simple scatter plot
            st.subheader("3. ROA vs ROE Scatter")
            
            # Sample data if too many points
            if len(df) > 100:
                plot_df = df.sample(100, random_state=42)
            else:
                plot_df = df
            ...
            fig3 = px.scatter(
                ...
                y='ROE',
                color='Cluster',
                size='Net_Margin' if 'Net_Margin' in df.columns else None,
                hover_data=['Ticker', 'Name', 'Sector'],
                title='ROA vs ROE by Cluster',
                opacity=0.7
            )
